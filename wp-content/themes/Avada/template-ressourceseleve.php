...
<section id="content" <?php Avada()->layout->add_style( 'content_style' ); ?>>
    ...
    <form method="GET" id="form">
		...
		<div class="row">
		   ...
		   <div class="col-6 row paddingright40">
				<div class="form-group row">
					<label class="col-12 col-form-label fontsize30" for="ressources-eleves">Matière :</label>
					<div class="col-12" >
					  <select multiple name="ressources-eleves[]" id="ressources-eleves" class="filter-multi-select">
						<?php foreach($parent_terms2 as $value2){ 
							...
							$selected="";
							if(isset($_GET["ressources-eleves"])){
								
								if(in_array($value2->term_id,$_GET["ressources-eleves"]))$selected="selected";
			...
			<?php if(isset($_GET["niveau"]) or isset($_GET["ressources-eleves"])){ ?>
			...
			<?php if(isset($_GET["niveau"]) or isset($_GET["ressources-eleves"])){ ?>
			<div class="col-6 row">
				<a id="filtrerbutcancel" href="<?php echo get_site_url(); ?>/eleves/ressources-par-niveaux-et-matieres/" class="btn btn-secondary" ><i class="fa fa-window-close-o" aria-hidden="true"></i>Annuler</a>
			</div>
    ...
    <div id="contentarticle">
		...
		if(isset($_GET["niveau"]) or isset($_GET["ressources-eleves"])){
			...
			if(empty($_GET["ressources-eleves"])){
				
				$_GET["ressources-eleves"]=$lstarray2;
				
			}
						
			$lstarrayfiltre=array_merge($_GET["niveau"], $_GET["ressources-eleves"]);
			...
			$args = array(
				'posts_per_page'   => -1,
				'category'         => $_GET["ressources-eleves"],
		...
		$cpt=0;
			foreach($posts as $post){
				...
				foreach($lstcategories as $value){
					...
					if($catobject->category_parent==$parentarray[1])$good2=1;
						
				}
				
				if(isset($_GET["niveau"],$_GET["ressources-eleves"])){
					...
					foreach($lstcategories as $value){
						...
						if(in_array($value,$_GET["ressources-eleves"]))$good1=1;
				...
				if($good==1 and $good1==1 and $good2==1){
					...
					<article>
						...
						foreach($lstcategories as $cate){
							...
							<a href="<?php echo get_site_url(); ?>/eleves/ressources-par-type-niveaux-matieres/?<?php echo $parent->slug; ?>%5B%5D=<?php echo $cate; ?>" class="categoryclass" style="background:<?php echo $rl_category_color; ?>;"><?php echo get_cat_name($cate); ?></a>
