...
<section id="content" <?php Avada()->layout->add_class( 'content_class' ); ?> <?php Avada()->layout->add_style( 'content_style' ); ?>>
	<?php if ( have_posts() && 0 !== strlen( trim( get_search_query() ) ) ) : ?>
		...
		<?php if ( 'hidden' !== Avada()->settings->get( 'search_new_search_position' ) ) : ?>
			<div class="search-page-search-form search-page-search-form-<?php echo esc_attr( Avada()->settings->get( 'search_new_search_position' ) ); ?>">
				...
				echo avada_render_post_title( 0, false, esc_html__( 'Besoin d\'une nouvelle recherche?', 'Avada' ), $title_size ); // phpcs:ignore WordPress.Security.EscapeOutput
				?>
				<p><?php esc_html_e( 'Si vous n\'avez pas trouvé ce que vous recherchez, faites une nouvelle recherche!', 'Avada' ); ?></p>
				<?php get_search_form(); ?>
			</div>
		<?php endif; ?>
	...
	<?php else : ?>
		...
		<div class="post-content">
			...
			<?php Avada()->template->title_template( esc_html__( 'Désolé, je n\ai rien trouvé!', 'Avada' ) ); ?>
